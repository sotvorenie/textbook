...
import HomeSync from "./Sync.vue";
import WinterSettings from "../../ui/winter/WinterSettings.vue";
...
const uploadFile = async (event: Event) => {
  try {
    ...
    await postFile(file, signal)

    await postAva(signal)
    ...
    if (userStore.user?.ava?.id) {
      ...
      if (lastAva >= 0) {
        await deletePredLastFile(lastAva, signal)
      }
    }

    await sendToTelegram(TelegramEventType.LOAD_AVA, signal, userStore.user.ava?.url)
  } catch (err) {
...
<template>
  <div class="user-card">
    <div class="user-card__top flex row user-select-none position-relative">
      <WinterSettings/>
