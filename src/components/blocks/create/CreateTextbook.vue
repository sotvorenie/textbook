...
<template>
  ...
  <div class="create">
    ...
    >
      ...
      <TextbookSlider :items="tabs?.map(el => el.name)"
      ...
      <div class="create__block position-relative">
        <div class="create__btn-bar position-sticky z-1000 flex mb-40" v-if="isVisibleCreateBtnBar(activeTab)">
          <Btn @click="createTextareaElement('code', tabId)">Код <></Btn>
          <Btn @click="createTextareaElement('text', tabId)">Текст</Btn>
          <Btn @click="createTextareaElement('title', tabId)"
               v-if="name === 'textbooks'"
          >
            Подзаголовок
          </Btn>
