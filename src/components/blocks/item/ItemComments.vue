...
import CommentsSkeleton from "./ItemCommentsSkeleton.vue";
...
const getData = async (getVisible: boolean = true) => {
  try {
    ...
    const data: {meta: any, items: Comment[]} =
        await getComments(id.value, props.name, page.value, signal)

    if (getVisible) {
      const visibleData: boolean | undefined =
          await checkComment(id.value, props.name, signal)
  ...
  try {
    ...
    if (redactedComment.value) {
      ...
      const comment: Comment | undefined =
          await setComment(id.value, props.name, commentText.value, signal)
...
const handleRemoveComment = async (id: number) => {
  ...
  if (check) {
    try {
      ...
      await removeComment(props.name, id, signal)
...
onMounted(async () => {
  ...
  if (mainElement.value && mainElement.value.scrollHeight !== mainElement.value.clientHeight) {
...
<template>
  ...
  <div class="comments position-relative">
    <p class="comments__title h4">Комментарии:</p>
          ...
          <div class="comments__content w-100">
            <div class="comments__top flex flex-justify-between">
              ...
              >
                ...
                <template #default>
                  <div class="comments__actions flex">
                    <Btn class="comments__actions-btn button-small mb-10"
                         @click="handleRedact(comment)"
                         :is-disabled="isLoading"
                    >
                      Редактировать
                    </Btn>
                    <Btn class="comments__actions-btn button-small"
